...
async def handle_client(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, banner: str):
...
async def run_tcp_honeypot(port: int, banner: str = "Welcome to legacy management service."):
    server = await asyncio.start_server(
        lambda r, w: handle_client(r, w, banner), "0.0.0.0", port
    )
